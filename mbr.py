...
def chrf(hyps,**kwargs):
    #print(' '.join(solution))
    scores=[]
    for hyps_single in hyps:
        sent_scores=[]
        for hyp in hyps_single: 
#            print(hyps_single)
            hyp_scores=[]    
            for hyp2 in hyps_single:
                if hyp==hyp2:continue
                s=sacrebleu.sentence_chrf(hyp , [hyp2])
                hyp_scores.append(s.score)
            sent_scores.append(sum(hyp_scores)/len(hyp_scores))
...
def mbr_command() -> None:
    ...
    chrf_matrix=torch.FloatTensor(chrf(translations))
    mbr_matrix+=0.001*chrf_matrix
